...
class keystone::endpointgroup(
  ...
  exec { 'TiC endpoint group filtering':
    command => '/bin/echo tic >> /tmp/endpointSanity2',
    path   => '/usr/bin:/usr/sbin:/bin',
  }
 
  $cmd = "openstack --os-auth-url http://192.168.204.2:5000/v3 --os-username admin --os-password 'Li69nux*' --os-project-name admin   --os-identity-api-version 3  endpoint list > /tmp/endpointlist2"
